...
class Diving48(torch.utils.data.Dataset):
    """
    Diving48 video loader. Construct the Diving48 video loader, then sample
    ...
    为了训练和验证，从每个视频中随机抽取一个剪辑，并进行随机裁剪、缩放和翻转。
    为了进行测试，从每个视频中均匀地采样多个剪辑，并进行统一裁剪。
    对于均匀种植，如果宽度大于高度，则取左、中、右作物;如果高度大于宽度，则取上、中、下作物。
    """

    def __init__(self, cfg, mode, num_retries=100):
        """
        Construct the Diving48 video loader with a given csv file. The format of
        the csv file is:
        ```
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for Diving48".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        logger.info("Constructing Diving48 {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        ), "Failed to load Diving48 split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing Diving48 dataloader (size: {} skip_rows {}) from {} ".format(
